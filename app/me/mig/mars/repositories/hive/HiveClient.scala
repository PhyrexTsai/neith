...
class HiveClient @Inject()(configuration: Configuration) {
  ...
  private val conn: Connection = DriverManager.getConnection(config.getString("jdbcUrl"), config.getString("user"), config.getString("password"))
  val isExist: Boolean = conn != null

  private val stmt: PreparedStatement = conn.prepareStatement(SELECT_SCHEDULED_JOB_USERS)
